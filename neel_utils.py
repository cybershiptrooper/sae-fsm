...
def get_induction_scores(model, make_plot=False, batch_size=4, ind_seq_len=200):
    ...
    random_tokens = rand_tokens_vocab[
        torch.randint(0, len(rand_tokens_vocab), (batch_size, ind_seq_len))
    ]
    bos_tokens = torch.full((batch_size, 1), model.tokenizer.bos_token_id, dtype=torch.long).cuda()
    ...
    ind_head_scores = einops.reduce(
        ...
        "layer batch head diag_pos -> layer head",
        "mean",
    )
    if make_plot:
